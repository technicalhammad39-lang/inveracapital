...
export default function ReferralsClient({ dbStats, dbTree, dbLeaderboard }: { dbStats?: any, dbTree?: any, dbLeaderboard?: any[] }) {
  const { formatCurrency } = useCurrency();
  const [copied, setCopied] = useState(false);
  const [copiedLink, setCopiedLink] = useState(dbStats?.code ? `${window.location.origin}/ref/${dbStats.code}` : '');
  const [showQr, setShowQr] = useState(false);

  const treeData = dbTree || {
    name: "You",
    level: 0,
    active: false,
    team: 0,
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: 'Network Count', value: dbStats?.totalTeam || 0, icon: Users, isPositive: true },
          { label: 'Active Partners', value: dbStats?.activeTeam || 0, icon: TrendingUp, isPositive: true },
          { label: 'Total Earnings', value: dbStats?.totalCommissions || 0, icon: Award, isPositive: true, currency: true }
        ].map((stat, i) => {
            ...
            >
              <div className="p-5 flex flex-col gap-4">
                <div className="flex items-start justify-between">
                  ...
                  <Icon className="w-6 h-6 text-brand transition-transform duration-300 shrink-0 opacity-80" />
                </div>
          ...
          <table className="w-full text-xs text-left min-w-[600px]">
            ...
            <tbody className="divide-y divide-border/60">
              {(dbLeaderboard || []).length > 0 ? (
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-10 text-center text-text-secondary font-semibold">
