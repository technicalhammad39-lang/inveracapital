...
import { claimReward } from '@/app/actions/rewardActions';
...
export default function RewardsClient({ dbData }: { dbData?: any }) {
  const { formatCurrency } = useCurrency();
  
  // Use real DB data or mock fallback
  const achievements = dbData?.milestones || mockAchievements;
  const currentDayState = dbData?.daily?.currentDay || 3;
  const [currentDay, setCurrentDay] = useState(currentDayState);

  // Daily login rewards state
  const [dailyClaimed, setDailyClaimed] = useState<Record<number, boolean>>(
    dbData?.daily?.claimed || { 1: true, 2: true, 3: false, 4: false, 5: false, 6: false, 7: false }
  );
  
  const [milestonesClaimed, setMilestonesClaimed] = useState<Record<string, boolean>>(
    achievements.reduce((acc: any, ach: any) => ({ ...acc, [ach.id]: !!ach.isClaimed }), {})
  );

  const [claimToast, setClaimToast] = useState<string | null>(null);

  const dailyAmounts = dbData?.daily?.amounts || [1, 1.5, 2, 2.5, 3, 5, 10]; // Rewards value
  const dailyIds = dbData?.daily?.ids || [];

  const handleClaimDaily = async (day: number) => {
    // Can only claim if previous days are claimed and this day is not claimed
    if (day !== currentDay || dailyClaimed[day]) return; 
    
    try {
      const rewardId = dailyIds[day - 1];
      if (rewardId) {
      ...
      // Advance current day if claimed
      setCurrentDay((prev: number) => prev >= 7 ? 1 : prev + 1);
  ...
  const handleClaimMilestone = async (ach: Achievement) => {
    if (ach.current < ach.target || milestonesClaimed[ach.id]) return;

    try {
      const res = await claimReward(ach.id);
      if (!res.success) {
         alert(res.error);
         return;
      }
      setMilestonesClaimed(prev => ({ ...prev, [ach.id]: true }));
      setClaimToast(`Milestone Reward of ${formatCurrency(ach.reward)} added to Bonus Wallet!`);
      setTimeout(() => setClaimToast(null), 3000);
    } catch (e) {
      console.error(e);
    }
    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="glass p-6 rounded-3xl relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
          {dailyAmounts.map((amt: any, idx: number) => {
            const dayNum = idx + 1;
        ...
        <div className="glass p-6 rounded-2xl lg:col-span-2 space-y-6">
          ...
          <div className="space-y-4">
            {achievements.map((ach: any) => {
              ...
              const isClaimed = milestonesClaimed[ach.id];
              return (
                <div key={ach.id} className="bg-bg-base/30 border border-border/80 p-5 rounded-2xl flex flex-col md:flex-row justify-between md:items-center gap-6 hover:border-brand/40 transition-colors">
                  ...
                  <div className="shrink-0 flex flex-col items-end gap-2 justify-center">
                    ...
                    {isClaimed ? (
                      <div className="flex items-center gap-1 text-[9px] font-extrabold px-2.5 py-1.5 bg-brand/10 border border-brand/20 text-brand rounded-lg uppercase tracking-wider">
                        <Check size={10} /> Claimed
                      </div>
                    ) : isCleared ? (
                      <button 
                        onClick={() => handleClaimMilestone(ach)}
                        className="bg-brand text-black font-extrabold text-[10px] px-3.5 py-1.5 rounded-lg hover:bg-brand-hover shadow-[0_0_10px_rgba(0,255,136,0.2)] transition-all uppercase tracking-wider"
                      >
                        Claim Reward
                      </button>
                    ) : (
